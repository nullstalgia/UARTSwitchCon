...
BTN_SL           = 0x0000000000004000
BTN_SR           = 0x0000000000008000
...
def send_packet(packet=[0x00,0x00,0x08,0x80,0x80,0x80,0x80,0x00], debug=False):
    if not debug:
        ...
        print(bytes_out)
        print(' '.join('{:02x}'.format(x) for x in bytes_out))
...
def force_sync():
    ...
    if byte_in == RESP_SYNC_START:
        ...
        if byte_in == RESP_SYNC_1:
            ...
            byte_in = read_byte()
            #if byte_in == 0x01 or byte_in == 0x02 or byte_in == 0x03:
...
ser = serial.Serial(port=args.port, baudrate=19200,timeout=1, dsrdtr=False, rtscts=False, xonxoff=False)
...
"""
p_wait(0.3)
if not send_cmd(BTN_ZL):
    print('Packet Error!')
else:
    print('Test Packet Success!')
...
#send_packet([0x00,0x00,0x08,0x80,0x80,0x80,0x80,0x00])
